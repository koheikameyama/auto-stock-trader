// scripts/walk-forward-squeeze-breakout.ts
/**
 * スクイーズブレイクアウト戦略 Walk-Forward 分析
 *
 ...
 *   npm run walk-forward:squeeze-breakout -- --max-pf
 */
...
import { runSqueezeBreakoutBacktest, precomputeSqueezeBreakoutDailySignals } from "../src/backtest/squeeze-breakout-simulation";
import {
  SQUEEZE_BREAKOUT_BACKTEST_DEFAULTS,
  generateSqueezeBreakoutParameterCombinations,
  SQUEEZE_BREAKOUT_PARAMETER_GRID,
} from "../src/backtest/squeeze-breakout-config";
import type { SqueezeBreakoutBacktestConfig, PerformanceMetrics } from "../src/backtest/types";
...
interface WindowResult {
  ...
  oosStart: string;
  oosEnd: string;
  bestIsParams: Partial<SqueezeBreakoutBacktestConfig>;
  isMetrics: PerformanceMetrics;
  oosMetrics: PerformanceMetrics | null;
}

interface ComboResult {
  params: Partial<SqueezeBreakoutBacktestConfig>;
  metrics: PerformanceMetrics;
}

function paramComboKey(params: Partial<SqueezeBreakoutBacktestConfig>): string {
...
function selectByRobustness(comboResults: Map<string, ComboResult>): ComboResult | null {
  const gridArrays: number[][] = [
    [...SQUEEZE_BREAKOUT_PARAMETER_GRID.atrMultiplier],
    [...SQUEEZE_BREAKOUT_PARAMETER_GRID.beActivationMultiplier],
    [...SQUEEZE_BREAKOUT_PARAMETER_GRID.trailMultiplier],
  ];
...
async function main() {
  ...
  console.log("スクイーズブレイクアウト戦略 Walk-Forward 分析");
  console.log("=".repeat(70));
  ...
  const paramCombos = generateSqueezeBreakoutParameterCombinations();
  ...
  const maxPrice = SQUEEZE_BREAKOUT_BACKTEST_DEFAULTS.maxPrice;
  ...
  const filterCfg = SQUEEZE_BREAKOUT_BACKTEST_DEFAULTS;
  ...
  for (let w = 0; w < windows.length; w++) {
    ...
    const isSignals = precomputeSqueezeBreakoutDailySignals(filterCfg, allData, isPrecomputed);
    ...
    for (const params of paramCombos) {
      ...
      const result = runSqueezeBreakoutBacktest(config, allData, vixArg, indexArg, isPrecomputed, isSignals);
    ...
    const oosSignals = precomputeSqueezeBreakoutDailySignals(filterCfg, allData, oosPrecomputed);

    const oosConfig: SqueezeBreakoutBacktestConfig = {
      ...SQUEEZE_BREAKOUT_BACKTEST_DEFAULTS,
      ...bestParams,
      startDate: oosStart,
      endDate: oosEnd,
    ...
    const oosResult = runSqueezeBreakoutBacktest(oosConfig, allData, vixArg, indexArg, oosPrecomputed, oosSignals);
