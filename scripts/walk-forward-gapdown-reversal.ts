// scripts/walk-forward-gapdown-reversal.ts
/**
 * ギャップダウンリバーサル戦略 Walk-Forward 分析
 *
 ...
 *   npm run walk-forward:gapdown-reversal -- --max-pf
 */
...
import { runGapDownReversalBacktest, precomputeGapDownReversalDailySignals } from "../src/backtest/gapdown-reversal-simulation";
import { GAPDOWN_REVERSAL_BACKTEST_DEFAULTS, generateGapDownReversalParameterCombinations } from "../src/backtest/gapdown-reversal-config";
import type { GapDownReversalBacktestConfig, PerformanceMetrics } from "../src/backtest/types";
...
interface WindowResult {
  ...
  oosStart: string;
  oosEnd: string;
  bestIsParams: Partial<GapDownReversalBacktestConfig>;
  isMetrics: PerformanceMetrics;
  oosMetrics: PerformanceMetrics | null;
}

interface ComboResult {
  params: Partial<GapDownReversalBacktestConfig>;
  metrics: PerformanceMetrics;
}

function paramComboKey(params: Partial<GapDownReversalBacktestConfig>): string {
...
async function main() {
  ...
  console.log("ギャップダウンリバーサル戦略 Walk-Forward 分析");
  console.log("=".repeat(70));
  ...
  const paramCombos = generateGapDownReversalParameterCombinations();
  ...
  const maxPrice = GAPDOWN_REVERSAL_BACKTEST_DEFAULTS.maxPrice;
  ...
  const filterCfg = GAPDOWN_REVERSAL_BACKTEST_DEFAULTS;
  ...
  for (let w = 0; w < windows.length; w++) {
    ...
    const isSignals = precomputeGapDownReversalDailySignals(filterCfg, allData, isPrecomputed);

    // IS: 全パラメータ組み合わせテスト
    const comboResults = new Map<string, ComboResult>();

    for (const params of paramCombos) {
      const config: GapDownReversalBacktestConfig = {
      ...
      const result = runGapDownReversalBacktest(config, allData, vixArg, indexArg, isPrecomputed, isSignals);
    ...
    const oosSignals = precomputeGapDownReversalDailySignals(filterCfg, allData, oosPrecomputed);

    const oosConfig: GapDownReversalBacktestConfig = {
      ...GAPDOWN_REVERSAL_BACKTEST_DEFAULTS,
      ...bestParams,
      startDate: oosStart,
      endDate: oosEnd,
    ...
    const oosResult = runGapDownReversalBacktest(oosConfig, allData, vixArg, indexArg, oosPrecomputed, oosSignals);
