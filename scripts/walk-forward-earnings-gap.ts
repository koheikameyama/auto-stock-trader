// scripts/walk-forward-earnings-gap.ts
/**
 * 決算ギャップ戦略 Walk-Forward 分析
 *
 * IS（In-Sample）6ヶ月 / OOS（Out-of-Sample）3ヶ月
 * 3ヶ月スライド × 6ウィンドウ = 24ヶ月
 *
 * Usage:
 *   npm run walk-forward:earnings-gap
 *   npm run walk-forward:earnings-gap -- --max-pf
 */

import dayjs from "dayjs";
import { prisma } from "../src/lib/prisma";
import { fetchHistoricalFromDB, fetchVixFromDB, fetchIndexFromDB, fetchEarningsFromDB } from "../src/backtest/data-fetcher";
import { precomputeSimData } from "../src/backtest/breakout-simulation";
import { runEarningsGapBacktest, precomputeEarningsGapDailySignals } from "../src/backtest/earnings-gap-simulation";
import { EARNINGS_GAP_BACKTEST_DEFAULTS, generateEarningsGapParameterCombinations, EARNINGS_GAP_PARAMETER_GRID } from "../src/backtest/earnings-gap-config";
import type { EarningsGapBacktestConfig, PerformanceMetrics } from "../src/backtest/types";
...
/** IS最低トレード数（決算ギャップはシグナル少なめなので緩めに） */
...
interface WindowResult {
  ...
  oosStart: string;
  oosEnd: string;
  bestIsParams: Partial<EarningsGapBacktestConfig>;
  isMetrics: PerformanceMetrics;
  oosMetrics: PerformanceMetrics | null;
}

interface ComboResult {
  params: Partial<EarningsGapBacktestConfig>;
  metrics: PerformanceMetrics;
}

function paramComboKey(params: Partial<EarningsGapBacktestConfig>): string {
...
function selectByRobustness(comboResults: Map<string, ComboResult>): ComboResult | null {
  const gridArrays: number[][] = [
    [...EARNINGS_GAP_PARAMETER_GRID.atrMultiplier],
    [...EARNINGS_GAP_PARAMETER_GRID.beActivationMultiplier],
    [...EARNINGS_GAP_PARAMETER_GRID.trailMultiplier],
  ];
...
async function main() {
  ...
  console.log("決算ギャップ戦略 Walk-Forward 分析");
  console.log("=".repeat(70));
  ...
  const paramCombos = generateEarningsGapParameterCombinations();
  ...
  console.log(`[data] ${rawData.size}銘柄（raw）, VIX ${vixData.size}日, N225 ${indexData.size}日, 決算日 ${earningsData.size}銘柄`);

  const maxPrice = EARNINGS_GAP_BACKTEST_DEFAULTS.maxPrice;
  ...
  const filterCfg = EARNINGS_GAP_BACKTEST_DEFAULTS;
  ...
  for (let w = 0; w < windows.length; w++) {
    ...
    const isSignals = precomputeEarningsGapDailySignals(filterCfg, allData, isPrecomputed, earningsData);

    // IS: 全パラメータ組み合わせテスト
    const comboResults = new Map<string, ComboResult>();

    for (const params of paramCombos) {
      const config: EarningsGapBacktestConfig = {
      ...
      const result = runEarningsGapBacktest(config, allData, earningsData, vixArg, indexArg, isPrecomputed, isSignals);
    ...
    if (!selected) {
      console.log(`  ⚠ IS期間でトレードが${MIN_IS_TRADES}件未満のためスキップ`);
      console.log("");
      continue;
    }
    ...
    const oosSignals = precomputeEarningsGapDailySignals(filterCfg, allData, oosPrecomputed, earningsData);

    const oosConfig: EarningsGapBacktestConfig = {
      ...EARNINGS_GAP_BACKTEST_DEFAULTS,
      ...bestParams,
      startDate: oosStart,
      endDate: oosEnd,
    ...
    const oosResult = runEarningsGapBacktest(oosConfig, allData, earningsData, vixArg, indexArg, oosPrecomputed, oosSignals);
