...
- 立花ログイン承認 (arm) は GU/PSC と同じく中央ゲート (`submitOrder` → `client.request` → `requireLoginArm()`) に委譲。未承認時は `isActive=false` + 電話番号/再開リンク付き通知。**ETF 独自の `loginArmedUntil` プリチェックは廃止** — TTL 10分の短命フラグを通常運用（DB セッション復元時）に要求すると、健全な日でも ETF だけ誤スキップするため（2026-06-12 本番で観測）
