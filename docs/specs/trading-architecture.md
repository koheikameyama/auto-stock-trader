...
### 決済の執行と記録（KOH-547 / KOH-550 / KOH-551）

決済は **全経路が成行**（`position-monitor` の `executeExitSell` は指値なしの成行売り、ブローカーSLも逆指値・成行執行）。

**執行フロー**（`executeExitSell`）:

1. SL逆指値を取消 → 取消できなければ決済見送り（成行売りが売付可能不足で弾かれるため）
2. 成行売り → **失敗したら close も通知もしない**（DBだけ閉じる「幻の決済」を防ぐ）
3. **ブローカーから実約定価格を取得して記録する**

**記録には必ずブローカーの実約定価格を使う。** 日足モデル上の想定価格（`checkPositionExit` の `exitPrice`）は「決済すべきか」の判定用であって「いくらで売れたか」ではなく、日中に大きく乖離しうる。実約定を取得できない場合のみモデル値へフォールバックし、その旨を Slack 警告する。モデル値は `exitSnapshot.modelExitPrice` に残しライブ↔BT乖離の追跡に使う。

**ディフェンシブ決済の発火条件**（`position-monitor`、当日の market-assessment のみ参照）:

| 条件 | 由来 |
|---|---|
| `sentiment === "crisis"` | 日経キルスイッチ（≤ -3%）/ CME乖離（≤ -3%） |
| `vix > 30` | BT の `processDefensive`（`regime === "crisis"`）と同一条件 |

当日分の評価が無い場合は発火させない（stale な前日値での誤発火を防ぐ。SLは板に生きているので保護は残る）。

**end-of-day では決済しない。** 17:00 JST は大引け後かつ立花の受付停止窓明けで当日決済が物理的に不可能なため、crisis 時の未決済ポジション残存の**検知＋警告のみ**を行う。
...
| > 30  | crisis（1ポジ・S）     | 1ポジション制限+ディフェンシブ決済（position-monitor が全ポジションを成行決済。KOH-551）|
...
「予算ベース数量（残高上限）」= `maxByBalance` は **買余力 × `POSITION_SIZING.BUYING_POWER_BUFFER`（0.80）** で計算する（`entry-executor.ts`）。

- **理由**: 立花は日計り取引の規制対象銘柄で買付可能額に掛目を効かせて算出するため、約定代金 vs 一般買余力で組んだ数量が発注時に `[sub:11430]`（買付可能額不足）で弾かれる。2026-07-22 の 8708.T は必要額が約定代金の約1.22倍だった。
- **効果**: バッファ 0.80 は掛目 1/0.80 = 1.25倍まで吸収する。通常銘柄（掛目1.0）には無害。
- **検証**: 16窓リセットBT（各窓¥500K）で 0.80 vs 現行1.00 に Calmar・NetRet とも有意差なし（Calmar t=+1.22 p=0.30、NetRet t=−0.40 p=1.00、全 p>0.3）。¥500Kでは現金が制約（`backtest.md` 却下#40）のため、小さく薄く回すとむしろ positive-edge trade を拾い Calmar は微プラス方向。
- **留保**: 掛目 >1.25倍の重規制銘柄は稀に依然拒否されうる（掛目の事前取得手段がないため完全保証は不可）。
...
**日本株 breadth < 54%（既存 GU/PSC が動作しない idle 帯）**でのみ JP上場の米株ETFにエントリーする補完戦略。既存戦略のシーズン性（D期 = 2-3ヶ月続く大強気でのみ大きく稼ぐ）を offseason 側から補完する目的で導入。

**設計の核**: 既存戦略 ON 時の ETF トレードは機会コストにしかならない。「既存戦略 OFF 時にだけ動かす」が補完戦略の正解（`.claude/rules/backtest.md` 「事例: 過去9年データ拡張検証 + 米株ETF補完戦略」参照）。

### ユニバース

| ティッカー | 銘柄名 | 連動指数 |
|---|---|---|
| 1547 | SPDR S&P500 ETF | S&P 500 |
| 1545 | NEXT FUNDS NASDAQ-100 連動型上場投信 | NASDAQ-100 |

立花証券 e支店で取引可能。¥30-40K/単元、1株単位。

### エントリー条件

| 条件 | 閾値 | 備考 |
|---|---|---|
| gap | ≥ 0.5%（`(today_open - prev_close) / prev_close`） | gapupより緩い |
| 出来高サージ | ≥ 1.5x（vs 25日平均） | |
| 陽線 | today_close > today_open | |
| 日本株 breadth | **< 54%** | **idle帯フィルター（必須）** |

### リスク管理

| 項目 | 値 |
|---|---|
| 損切（SL） | -2%（逆指値、立花API で寄付発注と同時セット） |
| タイムストップ | 5営業日 |
| リスク% | 資金の1.5%（GU/PSCより低め） |
| 1ポジション上限 | 資金の40% |
| エントリー時刻 | 翌営業日 8:35 JST（寄付前） |
| 注文形態 | 寄付成行 + SL逆指値同時（`sGyakusasiOrderType=2`） |
...
| 構成 | PF | Calmar | NetRet | MaxDD |
|---|---:|---:|---:|---:|
| ETF MVP（フィルターなし） | 1.39 | 4.12 | +32.4% | -7.85% |
| **ETF + idle帯フィルター（採用）** | **1.83** | **5.24** | +33.7% | **-6.43%** |
...
→ ETFの利益はほぼ全て idle 帯から発生。

### 補完戦略として除外している機構

シンプル維持のため、既存 GU/PSC が持つ以下のリスク制御は **適用しない**:

- 連敗スロットル
- VIX レジーム別 risk% scaling（elevated/high で縮小）
- セクター集中度（ETF は2銘柄しかないため）
- 連敗クールダウン

ただし以下は適用される:

- `TradingConfig.isActive=false` で全停止
- `US_ETF_RISK_PARAMS.entryEnabled=false` で戦略レベル停止
- 立花ログインロック (`loginLockedUntil`) 中は自動スキップ
- 立花ログイン承認 (arm) ゲートは**撤去済み（2026-06-29）**。v4r9 で電話番号認証が不要になった（6/27 以降）ため、`login()` 前の手動承認は廃止し production でも自動ログインする。立花が実際に 10089/10033 を返した場合のみ `handleAccountLock()` が `isActive=false` + 再開リンク付き通知を送る

### 実装ファイル

| ファイル | 役割 |
|---|---|
| `src/core/us-etf/entry-conditions.ts` | `detectUSEtfSignal()` シグナル判定、`US_ETF_RISK_PARAMS` パラメータ |
| `src/jobs/us-etf-monitor.ts` | **worker.ts node-cron 15:24**: エントリー（idle帯のみ立花リアルタイム値で評価→引け成行買い）+ タイムストップExit（5営業日経過→引け成行売り）+ `UsEtfSignal` 履歴記録。GU/PSC と同じ「当日引け約定」方式（BT前提一致） |
| `src/jobs/us-etf-health-check.ts` | 月次ヘルスチェック（`scheduled_monthly-strategy-health.yml` から呼ばれる） |
| `src/worker.ts` | 15:24:00/20/40 の node-cron に `us-etf-monitor` を配線（gapup/psc と並列） |
| `prisma/schema.prisma` | `UsEtfSignal` テーブル定義 |

> **2026-06-18 (KOH-498) で発注方式を刷新。** 旧構成（`us-etf-watchlist-builder.ts`(引け後検出) + `us-etf-entry-executor.ts`(翌朝寄付成行・GitHub Actions) + `us-etf-position-monitor.ts` + `scheduled_us-etf-entry.yml` / `scheduled_us-etf-monitor.yml`）は **CI起動オーバーヘッド約30分で寄付(09:00)を逃し立花が受付エラーを返す構造的バグ** があったため削除。GU/PSC と同じ worker.ts node-cron 15:24・引け成行に統一した。

### 本番有効化のチェックリスト

1. `Stock` テーブルに 1547 / 1545 が登録されていることを確認
2. 本番環境変数設定: `TACHIBANA_ENV=production`, `TACHIBANA_AUTH_ID`, `TACHIBANA_PRIVATE_KEY`, `TACHIBANA_SECOND_PASSWORD`, `ETF_TRADING_BUDGET`（Railway Worker）
3. PR マージ → Railway 再デプロイで worker.ts の `us-etf-monitor` (15:24) が稼働
4. **cron-job.org 「ETF Entry Executor」「ETF Position Monitor」を無効化（enabled=false）**（dispatch 先 workflow を削除済のため）
5. idle帯（breadth<54%）のシグナル発火日に、worker が 15:24 に引け成行発注し当日引けで約定することを観察
