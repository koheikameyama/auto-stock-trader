...
| `src/jobs/us-etf-watchlist-builder.ts` | 引け後シグナル検出 + `UsEtfSignal` DB保存 + Slack通知 |
| `src/jobs/us-etf-entry-executor.ts` | 翌営業日 寄付前 8:35 JST: 未執行シグナルを取得して立花API に寄付成行+SL逆指値同時発注 |
| `src/jobs/us-etf-position-monitor.ts` | 引け前 15:20 JST: 5営業日経過ポジションに引け成行売り発注 |
...
| `.github/workflows/scheduled_us-etf-entry.yml` | 寄付発注ワークフロー（**cron 未有効化、manual dispatch のみ**） |
| `.github/workflows/scheduled_us-etf-monitor.yml` | タイムストップ監視ワークフロー（**cron 未有効化**） |
| `.github/workflows/scheduled_backfill-prices.yml` | `us-etf-watchlist` ジョブを内包（既に日次稼働中） |

### 本番有効化のチェックリスト

1. `Stock` テーブルに 1547 / 1545 が登録されていることを確認
2. GitHub secrets 設定: `TACHIBANA_ENV`, `TACHIBANA_USER_ID/PASSWORD/SECOND_PASSWORD`, `ETF_TRADING_BUDGET`
3. デモ環境で `scheduled_us-etf-entry.yml` を `workflow_dispatch` で手動実行し、発注フローが通ることを確認（**注**: 立花デモ環境は売り注文がエラーになる仕様。買いのみ動作確認可）
4. シグナル発火日に本番環境で寄付発注が成功することを観察
5. `scheduled_us-etf-entry.yml` と `scheduled_us-etf-monitor.yml` の cron をアンコメント、または cron-job.org に `workflow_dispatch` トリガーを登録（時間厳密のため後者推奨、`.claude/rules/github-actions.md` 準拠）
