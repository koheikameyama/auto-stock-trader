...
投資スタイル（慎重派/バランス型/積極派）ごとの、おすすめ・購入分析・ポートフォリオ分析で適用される条件・閾値・ロジックの一覧。
...
| 項目 | 慎重派（CONSERVATIVE） | バランス型（BALANCED） | 積極派（AGGRESSIVE） |
|---|---|---|---|
| アイコン | 🛡️ | ⚖️ | 🚀 |
| ラベル | 慎重派（守り） | バランス型 | 積極派（攻め） |
...
| スタイル | 閾値 | 定数名 |
|---|---|---|
| 慎重派 | +20% | `MOMENTUM.CONSERVATIVE_SURGE_THRESHOLD` |
| バランス型 | +25% | `MOMENTUM.BALANCED_SURGE_THRESHOLD` |
| 積極派 | +50% | `MOMENTUM.AGGRESSIVE_SURGE_THRESHOLD` |
...
| スタイル | 閾値 | 定数名 |
|---|---|---|
| 慎重派 | -10% | `MOMENTUM.CONSERVATIVE_DECLINE_THRESHOLD` |
| バランス型 | -15% | `MOMENTUM.BALANCED_DECLINE_THRESHOLD` |
| 積極派 | -20% | `MOMENTUM.AGGRESSIVE_DECLINE_THRESHOLD` |
...
| 慎重派 | ブロック |
| バランス型 | ブロック |
| 積極派 | **スキップ**（`MOMENTUM.AGGRESSIVE_SKIP_OVERHEAT = true`） |
...
| スタイル | 閾値 | 定数名 |
|---|---|---|
| 慎重派 | 10% | `TIMING_INDICATORS.GAP_UP_SURGE_CONSERVATIVE` |
| バランス型 | 15% | `TIMING_INDICATORS.GAP_UP_SURGE_BALANCED` |
| 積極派 | 20% | `TIMING_INDICATORS.GAP_UP_SURGE_AGGRESSIVE` |
...
| スタイル | 閾値 | 定数名 |
|---|---|---|
| 慎重派 | 70% | `TECHNICAL_BRAKE.CONSERVATIVE` |
| バランス型 | 75% | `TECHNICAL_BRAKE.BALANCED` |
| 積極派 | 85% | `TECHNICAL_BRAKE.AGGRESSIVE` |
...
- 慎重派のみ volatility を**反転**（低ボラ = 高スコア）
...
- `isOverheated`（過熱圏）→ 積極派以外で除外
- 異常急騰: 週間 +50%超（積極派は +80%超）→ 完全除外
...
3. **過熱圏**: `isOverheated()` → buy→stay、confidence に -0.15 ペナルティ（積極派はスキップ）
...
| スタイル | 撤退ライン率 | 売却目標率 |
|---|---|---|
| 慎重派 | 0.03〜0.10 | 0.05〜0.20 |
| バランス型 | 0.05〜0.15 | 0.10〜0.40 |
| 積極派 | 0.07〜0.20 | 0.15〜0.50以上 |

### 4-5. 積極派リバウンド狙いロジック

> ソース: `lib/constants.ts:452-466`

慎重派・バランス型が stay でも、以下の条件で積極派のみ buy に昇格:
...
| 既にbuyの積極派への confidence ブースト | +0.05 |
...
- 積極派の confidence は他スタイルより高めに設定可能
...
|---|---|---|---|
| 慎重派 | +3%以上 | 75% | `PROFIT_TAKING_PROMOTION.CONSERVATIVE_*` |
| バランス型 | +8%以上 | 50% | `PROFIT_TAKING_PROMOTION.BALANCED_*` |
| 積極派 | +15%以上 | 25% | `PROFIT_TAKING_PROMOTION.AGGRESSIVE_*` |
