...
**ペア間は `\x01`（SOH）、キーと値の間は `\x02`（STX）** で区切られた文字列（2026-07-06 デモ実測で確定）:

```
p_no\x0210\x01p_date\x022026.07.06-17:44:11.367\x01p_cmd\x02KP\x01
```

> **⚠️ 過去の誤記**: 旧版の本ドキュメントは「`\x01` 区切りでキー・値が交互」と記載していたが誤り。
> この誤記に基づく旧パーサーは `p_cmd` を一切抽出できず、全メッセージ破棄 → KPタイマー未リセット →
> 30秒毎の再接続ループ + EC約定通知の全取りこぼしが発生していた（KOH-528）。

サーバーエラーは `p_errno`（`"0"` 以外）+ `p_err` で通知される（例: `p_errno=2, p_err="session inactive."`）。

### 8.4 イベント種別

| コード | 名称 | 説明 |
|--------|------|------|
| `KP` | Keep-alive | **5秒間隔**で送信（2026-07-06 デモ実測。旧記載の15秒は誤り） |
| `EC` | Execution Confirmation | 約定通知。`p_ON`（注文番号）+ `p_ED`（営業日 YYYYMMDD）で注文特定 |
| `ST` | Status | ステータス通知（エラー通知にも使用） |
| `SS` | System Status | システムステータス通知 |
| `US` | User Status | 運用ステータス通知（`p_US` = 運用ステータスコード） |
| `FD` | Feed | 時価配信（本システム未購読） |
| `NS` | News | ニュース（本システム未購読） |

### 8.5 約定通知（EC）の処理

1. ECイベント受信 → `p_ON`（注文番号）と `p_ED`（営業日）を取得
2. `CLMOrderListDetail` APIで約定詳細を取得
3. `aYakuzyouSikkouList` から約定情報を解析
4. 複数部分約定がある場合は加重平均価格を算出

主なECフィールド（参考: go-tachibanaapi 実装）: `p_ON`=注文番号 / `p_ED`=営業日 / `p_OON`=親注文番号 / `p_IC`=銘柄コード / `p_MC`=市場 / `p_BBKB`=売買区分 / `p_NT`=通知種別 / `p_EXPR`=約定値段 / `p_EXSR`=約定数量 / `p_EXST`=約定ステータス

### 8.6 接続管理

- **接続時間帯**: JST 07:00〜18:00（営業日のみ）
- **キープアライブタイムアウト**: 30秒（KP送信間隔5秒の6回分。heartbeat==timeout アンチパターンを避けジッタ耐性を持たせる）
