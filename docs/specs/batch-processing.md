...
4. **AI再評価**: `reassessMarketMidday()` で前場実績に基づく再評価
5. **Sentinel判定**: センチメントが悪化方向の場合のみ `sentiment` フィールドを更新
...
- **Read**: `MarketAssessment`, `TradingOrder`（pending buy）
- **Write**: `MarketAssessment`（midday fields + potentially sentiment）, `TradingOrder`（cancelled）
