...
| ステータス | 条件 | バッジ色 |
|---|---|---|
| 監視中（Cold） | デフォルト状態 | グレー |
| Hot | 出来高サージ比率 ≥ 1.5x | オレンジ |
| Triggered | 本日トリガー発火済み | 青 |
| 保有中 | 既にポジションあり | 緑 |

**ソート順**: Triggered → Hot → 保有中 → 監視中。同一ステータス内ではサージ比率降順。

**データ取得**: breakout-monitor のインメモリ BreakoutScanner 状態を `getScannerState()` で直接参照（同一プロセス）。市場時間外はスキャナー未起動のため全銘柄「監視中」表示。
