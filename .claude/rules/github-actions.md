...
| スケジューラ | 用途 | 対象ジョブ |
|-------------|------|-----------|
| **cron-job.org** | 取引時間に連動し、時間の正確性が重要な処理 | morning-analysis, order-manager, midday-reassessment, end-of-day |
| **GitHub Actions cron** | 閉場後・週末など、数分〜数十分のズレが許容される処理 | ghost-review, defensive-exit-followup, unfilled-order-followup, daily-backtest, jpx-delisting-sync, weekly-review, scoring-accuracy-report |
