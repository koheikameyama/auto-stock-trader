...
| シグナル検出 | `src/jobs/us-etf-watchlist-builder.ts` | 引け後、日本株breadth計算→1547/1545チェック→`UsEtfSignal` 保存 + Slack |
| 自動発注 | `src/jobs/us-etf-entry-executor.ts` | 寄付成行 + SL逆指値、立花API、`ETF_TRADING_BUDGET` 制御 |
| タイムストップ exit | `src/jobs/us-etf-position-monitor.ts` | 15:20、5営業日経過で引け成行売り |
| 月次ヘルスチェック | `src/jobs/us-etf-health-check.ts` | `scheduled_monthly-strategy-health.yml` に統合 |
| cron 配線 | `scheduled_us-etf-entry.yml` / `scheduled_us-etf-monitor.yml` / `scheduled_backfill-prices.yml`(watchlist相乗り) | - |
| DBスキーマ | `prisma/schema.prisma` の `UsEtfSignal` | - |
| データbackfill | `scripts/backfill-us-etf-bars.py` | 1547.T/1545.T を 2018- から取得 |

- **残作業**: combined BT (`src/backtest/combined-simulation.ts`) への統合は未実施。ETF 単独の検証は `scripts/_us-etf-{backtest-mvp,combined-mvp}.ts` / `scripts/_walk-forward-us-etf.ts` で完了済みのため運用には支障なし。GU+PSC+ETF を共有資金プールで一括検証したい場合のみ統合が必要
