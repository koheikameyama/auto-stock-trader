...
| **米株ETF idle帯補完 (1547+1545)** | **堅牢✓ (OOS PF 1.91, MaxDD -6.4%, Calmar 5.24)** | **本番投入Pending (A-4 実装待ち)** | **¥500K〜現運用規模で即採用可**。エントリー条件: 日本株 breadth < 54% (idle 帯) のみ + gap ≥ 0.5% + vol ≥ 1.5x + 陽線。SL -2%, タイムストップ 5日。8.5年BTで PF 1.83, 累計+33.7%。2018-2026 のうち **既存戦略OFF時 (idle 帯) で利益の大半 (35/32) が生まれる構造的補完性を確認**。立花API で 1547/1545 取引可、コストゼロで追加可能 |
...
- ロット: 1株単位、¥30-40K/単元
- A-4 実装 (entry-executor, watchlist-builder, cron 配線) は別セッション
