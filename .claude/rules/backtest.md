...
| **自社株買いカタリスト (KOH-502)** | **単独8年8勝 (idle帯 PF 2.11 / 期待値+2.73% / 勝率59%, コスト0.5%往復)** | **本番投入Pending** | **¥10M+運用時**。TDnet「自己株式取得に係る事項の決定」を idle帯(breadth<54%)で開示翌営業日引けエントリー、-12%固定SL + 20営業日タイムストップ。**却下リスト史上初の「価格と無相関・offseason稼働」エッジ**。¥500K combined では GapUp(PF4.4)の資金を食い Calmar -38% で不合格だが、**¥10M combined では Calmar +3% / MaxDD -1.4pp改善 / 稼働率+10pp で合格**（遊休現金をリスク増なしで埋める）。MOM/WB/ETFと違い MaxDD を"改善"する数少ない補完。詳細は下記「事例: 自社株買いカタリスト戦略の検証」参照 |
...
**判定: ¥10M+ で Pending(本物のエッジ、現運用規模では投入しない)。** buyback単独はプール内でも黒字(¥500K: 28tr, 勝率68%, PF 2.41)だが、**16.5日保有ポジが idle→band 転換後もGapUpの資金を握り、GapUp(PF4.4)を飢えさせる**(#28と同型の資金食い合い)。ただし**資金競合ダメージは資金量で激減**(NetRet毀損 ¥500K -66pp → ¥10M -14pp)、¥10Mでは遊休現金(約79%)を買いが埋め**idle帯の無相関ポジがoffseasonのDDを埋めてMaxDDを改善** = Calmar微プラス。**MOM/WB/ETFと違い MaxDD を"改善"する数少ない補完**。却下でなくMOM LC/WB LCと同じ「¥10M+で条件付き採用」バケット。
...
- **出口思想はエッジの性質に合わせる**: 遅いドリフト型にmomentum型のタイトトレーリングは致命的。辛抱型には広いカタストロフ損切り+タイムストップ
- **共有プールでは"良いだけ"では負ける**: 単独PF 2.4でも、GapUp(PF4.4)が全資金を回す複利局面では資金を食うだけ。補完戦略の価値は**遊休現金が制約になる大資金規模**で初めて出る(#28の教訓の裏返し)
