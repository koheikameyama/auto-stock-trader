...
export const POSITION_SIZING = {
  ...
  /**
   * 買余力バッファ（KOH-580）: maxByBalance を「買余力 × この係数」で計算する。
   * 立花は日計り取引の規制対象銘柄で買付可能額に掛目を効かせて算出するため、約定代金 vs 一般買余力で
   * 組んだ数量が発注時に [sub:11430] で弾かれる（2026-07-22 8708.T は必要額が約定代金の約1.22倍）。
   * 0.80 は掛目 1/0.80 = 1.25倍まで吸収する。16窓リセットBTで Calmar への有意なコストなしを確認済
   * （0.80 vs 現行1.00: Calmar t=+1.22 p=0.30 / NetRet t=-0.40 p=1.00、全 p>0.3）。
   */
  BUYING_POWER_BUFFER: 0.8,
