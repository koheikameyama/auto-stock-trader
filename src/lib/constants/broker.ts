...
/** p_no（リクエスト連番）順序エラーの自己修復設定。
 *
 * 立花のセッションは常駐 worker と cron ジョブ（end-of-day 等）が DB 経由で共有し、
 * サーバは全クライアント横断で p_no の単調増加を要求する。あるプロセスが多数の
 * リクエストで p_no を進めると、サーバの最終 p_no は wall-clock 秒より先行し、
 * `Date.now()/1000` で seed する新規プロセスは常にサーバより後ろから始まる。
 *
 * エラー時に読み取ったサーバ最終 p_no ちょうどに合わせて +1 だけ先行させても、
 * 並行プロセスのバースト（実測 ~18 req/秒）に即座に追い抜かれてリトライを使い切る。
 * そこで修正時は SAFETY_MARGIN 分だけ上乗せして先行し、以後しばらくは fix 無しで
 * 走れるようにする（p_no は timestamp 規模で実質無制限のため上乗せは無害）。 */
export const TACHIBANA_PNO = {
  /** p_no順序エラー修復時にサーバ最終p_noへ上乗せする余裕。
   * ~18 req/秒のバーストでも 1000/18 ≈ 55秒の先行を確保でき、1ジョブ分の
   * API呼び出しは 1回の修復で走り切れる。 */
  FIX_SAFETY_MARGIN: 1000,
  /** p_no順序エラー時の最大リトライ回数。並行プロセスが修復直後に追い抜くケースに備える。 */
  MAX_RETRIES: 5,
} as const;
