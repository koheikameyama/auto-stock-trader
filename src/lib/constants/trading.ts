...
/**
 * GU/PSC 戦略ユニバースの最大株価（円）
 *
 * BT検証で「エッジは低価格帯(≤¥2,500)に集中、上げるとノイズでPF低下＝資金が増えてもユニバースを拡大しない」
 * と確定（.claude/rules/backtest.md「資金スケーリング検証」/ 2026-07-03 maxPrice スイープ）。
 * 動的な getMaxBuyablePrice(資金) は集中率保証のため資金増で ¥2,500 を超えて広がるので、
 * ユニバースはこの固定値でキャップし BT のエッジ前提に固定する（≒¥543,600 未満では動的値の方が小さく無影響）。
 */
export const STRATEGY_UNIVERSE_MAX_PRICE = 2_500;
