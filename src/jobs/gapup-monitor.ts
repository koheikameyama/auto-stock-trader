...
export async function main(): Promise<void> {
  ...
  try {
    ...
    const { triggers: gapupTriggers, skipped: skippedByHolding } = gapupScanner.scan(
      gapupQuotes,
      holdingTickers,
    );

    // シグナルは満たしたが保有中/当日発注済み/決済後cooldownで発注しなかった候補も弾き分析に載せる
    // （とくに決済後cooldown=3営業日は BT パラメータなので、取り逃しの実額を後から測れるようにする）
    ...
    if (slotsLeft <= 0) {
      const reason = `枠上限（保有 ${guOpenCount} + 発注中 ${guPendingCount} / ${TRADING_DEFAULTS.MAX_POSITIONS_GU}）で候補を評価せず見送り`;
      console.log(`${tag} GU${reason}`);
      // 候補があったのに注文に乗らなかった = 弾き分析の対象
      await recordSkippedCandidates(gapupTriggers, "gapup", reason);
      lastScanDate = today;
      return;
    }

    let anyRetryable = false;
    for (let i = 0; i < gapupTriggers.length; i++) {
      const trigger = gapupTriggers[i]!;
      if (slotsLeft <= 0) {
        // 枠を使い切ったら当日確定。残り候補は未評価のまま見送るので弾き分析に載せる
        await recordSkippedCandidates(
          gapupTriggers.slice(i),
          "gapup",
          `枠上限（GU ${TRADING_DEFAULTS.MAX_POSITIONS_GU}枠を使い切り）で候補を評価せず見送り`,
        );
        break;
      }
      console.log(
      ...
      try {
        ...
        if (/最大同時保有数|現金残高不足|予算不足|日次損失制限/.test(reason)) {
          ...
          await recordSkippedCandidates(gapupTriggers.slice(i + 1), "gapup", reason);
          break;
        }
