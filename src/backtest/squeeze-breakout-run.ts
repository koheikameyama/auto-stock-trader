/**
 * スクイーズブレイクアウト バックテスト実行スクリプト
 *
 * Usage:
 *   npm run backtest:squeeze-breakout
 *   npm run backtest:squeeze-breakout -- --start 2025-04-01 --end 2026-03-25
 *   npm run backtest:squeeze-breakout -- --verbose
 *   npm run backtest:squeeze-breakout -- --compare-entry
 */

import dayjs from "dayjs";
import { prisma } from "../lib/prisma";
import { SQUEEZE_BREAKOUT_BACKTEST_DEFAULTS } from "./squeeze-breakout-config";
import { getMaxBuyablePrice } from "../core/risk-manager";
import { runSqueezeBreakoutBacktest } from "./squeeze-breakout-simulation";
...
import type { SqueezeBreakoutBacktestConfig, PerformanceMetrics } from "./types";
...
const ENTRY_COMPARISON_GRID: EntryComparisonRow[] = [
  { label: "sq10%+vol1.5x",  bbSqueezePercentile: 10, volSurgeRatio: 1.5 },
  { label: "sq15%+vol1.5x",  bbSqueezePercentile: 15, volSurgeRatio: 1.5 },
  { label: "sq20%+vol1.5x",  bbSqueezePercentile: 20, volSurgeRatio: 1.5 },
  { label: "sq25%+vol1.5x",  bbSqueezePercentile: 25, volSurgeRatio: 1.5 },
  { label: "sq30%+vol1.5x",  bbSqueezePercentile: 30, volSurgeRatio: 1.5 },
  { label: "sq20%+vol1.0x",  bbSqueezePercentile: 20, volSurgeRatio: 1.0 },
  { label: "sq20%+vol2.0x",  bbSqueezePercentile: 20, volSurgeRatio: 2.0 },
  { label: "brd60%+sq20%",   bbSqueezePercentile: 20, volSurgeRatio: 1.5, marketTrendThreshold: 0.6 },
  { label: "brd70%+sq20%",   bbSqueezePercentile: 20, volSurgeRatio: 1.5, marketTrendThreshold: 0.7 },
];

function runEntryComparison(
  baseConfig: SqueezeBreakoutBacktestConfig,
  ...
  for (const row of ENTRY_COMPARISON_GRID) {
    const config: SqueezeBreakoutBacktestConfig = {
      ...baseConfig,
      bbSqueezePercentile: row.bbSqueezePercentile,
      volSurgeRatio: row.volSurgeRatio,
      marketTrendThreshold: row.marketTrendThreshold ?? baseConfig.marketTrendThreshold,
      verbose: false,
    };
    const result = runSqueezeBreakoutBacktest(config, allData, vixData, indexData);
...
async function main() {
  ...
  const budget = Number(getArg(args, "--budget") ?? SQUEEZE_BREAKOUT_BACKTEST_DEFAULTS.initialBudget);
  const verbose = args.includes("--verbose");
  const compareEntry = args.includes("--compare-entry");
  const noPositionCap = args.includes("--no-position-cap");

  console.log("=".repeat(60));
  console.log("スクイーズブレイクアウト バックテスト");
  console.log("=".repeat(60));
  ...
  const baseConfig: SqueezeBreakoutBacktestConfig = {
    ...SQUEEZE_BREAKOUT_BACKTEST_DEFAULTS,
    startDate,
    endDate,
  ...
  // デフォルト実行
  const result = runSqueezeBreakoutBacktest(baseConfig, allData, vixArg, indexArg);
  printResult(result, "スクイーズブレイクアウト戦略");
...
main().catch((err) => {
  console.error("スクイーズブレイクアウトBTエラー:", err);
  process.exit(1);
});
