/**
 * ギャップダウンリバーサルバックテスト・シミュレーションエンジン
 *
 * 日次ループでギャップダウンリバーサルシグナルを検出し、当日終値でエントリー。
...
import { GAPDOWN_REVERSAL_RISK_PER_TRADE_PCT } from "./gapdown-reversal-config";
import { isGapDownReversalSignal } from "../core/gapdown-reversal/entry-conditions";
...
import type {
  GapDownReversalBacktestConfig,
  GapDownReversalBacktestResult,
  SimulatedPosition,
  DailyEquity,
  RegimeLevel,
} from "./types";
...
/**
 * ギャップダウンリバーサルシグナルの事前計算結果（1銘柄分）
 */
export interface PrecomputedGapDownReversalSignal {
  ticker: string;
  entryPrice: number;
  gapPct: number;
  atr14: number;
...
export type PrecomputedGapDownReversalSignals = Map<string, PrecomputedGapDownReversalSignal[]>;

/**
 * ギャップダウンリバーサルシグナルを一括事前計算する。
 * WFでは IS/OOS それぞれ1回呼んで全コンボに渡す。
 */
export function precomputeGapDownReversalDailySignals(
  config: Pick<GapDownReversalBacktestConfig,
...
): PrecomputedGapDownReversalSignals {
  const result: PrecomputedGapDownReversalSignals = new Map();
  const { tradingDays, dateIndexMap, dailyIndexAboveSma } = precomputed;

  for (let dayIdx = 0; dayIdx < tradingDays.length; dayIdx++) {
    const today = tradingDays[dayIdx];

    // マーケットフィルター（indexフィルターのみ）
    if (config.indexTrendFilter && !dailyIndexAboveSma.get(today)) continue;

    const daySignals: PrecomputedGapDownReversalSignal[] = [];
...
/**
 * ギャップダウンリバーサルバックテストを実行する
 */
export function runGapDownReversalBacktest(
  config: GapDownReversalBacktestConfig,
  allData: Map<string, OHLCVData[]>,
  vixData?: Map<string, number>,
  indexData?: Map<string, number>,
  precomputed?: PrecomputedSimData,
  precomputedSignals?: PrecomputedGapDownReversalSignals,
): GapDownReversalBacktestResult {
  ...
  if (precomputed) {
    ...
    // シグナルも計算
    precomputedSignals = precomputeGapDownReversalDailySignals(config, allData, computed);
  }
  ...
  for (let dayIdx = 0; dayIdx < tradingDays.length; dayIdx++) {
    ...
    if (todayRegime !== "crisis" && openPositions.length < config.maxPositions) {
      ...
      for (const signal of signals) {
        ...
        const riskAmount = cash * (GAPDOWN_REVERSAL_RISK_PER_TRADE_PCT / 100);
