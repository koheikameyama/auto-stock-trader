/**
 * 決算ギャップバックテスト・シミュレーションエンジン
 *
 * gapup-simulation.ts をベースに、決算日ゲートを追加。
...
import { EARNINGS_GAP_RISK_PER_TRADE_PCT } from "./earnings-gap-config";
...
import type {
  EarningsGapBacktestConfig,
  EarningsGapBacktestResult,
  SimulatedPosition,
  DailyEquity,
  RegimeLevel,
} from "./types";
...
/**
 * 決算ギャップシグナルの事前計算結果（1銘柄分）
 */
export interface PrecomputedEarningsGapSignal {
  ticker: string;
  entryPrice: number;
  gapPct: number;
  atr14: number;
...
export type PrecomputedEarningsGapSignals = Map<string, PrecomputedEarningsGapSignal[]>;

/**
 * 決算ギャップシグナルを一括事前計算する。
 * gapup版と同じだが、決算日のみに限定。
 */
export function precomputeEarningsGapDailySignals(
  config: Pick<EarningsGapBacktestConfig,
  ...
  earningsMap: Map<string, Set<string>>,
): PrecomputedEarningsGapSignals {
  const result: PrecomputedEarningsGapSignals = new Map();
  ...
  for (let dayIdx = 0; dayIdx < tradingDays.length; dayIdx++) {
    ...
    const daySignals: PrecomputedEarningsGapSignal[] = [];
    ...
    for (const [ticker, bars] of allData) {
      ...
      // 決算日ゲート: 当日が決算日であること
      const tickerEarnings = earningsMap.get(ticker);
      if (!tickerEarnings || !tickerEarnings.has(today)) continue;
 ...
 * 決算ギャップバックテストを実行する
 */
export function runEarningsGapBacktest(
  ...
  precomputed?: PrecomputedSimData,
  precomputedSignals?: PrecomputedEarningsGapSignals,
): EarningsGapBacktestResult {
  ...
  if (precomputed) {
    ...
    precomputedSignals = precomputeEarningsGapDailySignals(config, allData, computed, earningsMap);
  }
  ...
  for (let dayIdx = 0; dayIdx < tradingDays.length; dayIdx++) {
    ...
    if (todayRegime !== "crisis" && openPositions.length < config.maxPositions) {
      ...
      for (const signal of signals) {
        ...
        const riskAmount = cash * (EARNINGS_GAP_RISK_PER_TRADE_PCT / 100);
        ...
        if (config.verbose) {
          console.log(
            `  [${today}] ${signal.ticker} エントリー（決算Gap）: ¥${signal.entryPrice} x${finalQuantity}` +
