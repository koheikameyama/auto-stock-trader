 ...
 *
 * ---
 * 「トレーリングストップ」の実体について（KOH-548。直す前に必ず読むこと）
 *
 * 3 の発火条件は `bar.low <= effectiveSL` だが、この bar.low は
 *   - BT : その日の日足の安値
 *   - 本番: 立花時価の「当日ここまでの安値」（position-monitor が quote.low を渡す）
 * であり、いずれも **高値より前に付けた安値でも発火する**。
 * effectiveSL は当日高値から作られるため、実際には
 *   「当日レンジがトレール幅(0.5〜1.5×ATR)を超えたら次のチェックで手仕舞う」
 * という挙動になっている。名前が示すような「高値から追いかけるストップ」ではない。
 *
 * 実例 (2026-07-15 本番 8008.T): 09:05 安値2162 → 09:25 高値2195 → トレール2177。
 * 高値以降の安値は2182でトレールは一度も割れていないが、09:05 の安値が発火させて
 * 09:35 に 2183 で成行売りされた（結果的にその後2156まで下げたので得はした）。
 *
 * これは意図せずそうなった挙動だが、直すと成績は悪化する。発火条件を「高値以降の安値」に
 * 厳密化した close-confirmed モデルで測ると Calmar 22.98 → 15.00 (-35%)。
 * 却下リスト #37「早い建値ガードこそがDD防御の主役」/ #38「売った銘柄は決済後も市場に対し
 * 続落する」とも整合し、この戦略では早期手仕舞いが正解。**"正しいトレール"に直さないこと。**
 */
...
export interface PositionForExit {
  ...
  /**
   * SL約定価格のイントラバー・モデル（既定 "stop-at-open"）。
   *
   * "end-of-bar": 旧既定。KOH-548 で先読みと判明したため既定から外した。回帰比較用に残置。
   *   当日高値で切り上げた後のストップ（effectiveSL）と始値を比較し、
   ...
   *
   * "stop-at-open"（既定）: 始値時点で実在したストップ（前日までの最高値ベース）で
...
export function checkPositionExit(
  ...
  if (bar.low <= effectiveSL) {
    const model = position.intraBarStopModel ?? "stop-at-open";
    if (model !== "end-of-bar") {
